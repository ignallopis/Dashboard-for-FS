...
def _build_tv_figures(res: dict, x_mode: str = "laps") -> list[go.Figure]:
    ...
    if res["yaw_ok"].any():
        add_lap_scatter(
            fig,
            x_yaw,
            res["yaw_rmse"][res["yaw_ok"]][order_yaw],
            lap_list[res["yaw_ok"]][order_yaw],
        )
        if x_mode == "laps":
    ...
    if res["mz_ok"].any():
        add_lap_scatter(
            fig, x_mz, res["mz_rmse"][res["mz_ok"]][order_mz], lap_list[res["mz_ok"]][order_mz]
        )
        if x_mode == "laps":
    ...
    x_ratio, order_ratio, xlabel_ratio = per_lap_axis_or_empty(
        lap_list, lt, x_mode, res["ratio_ok"]
    )
...
def tv_figs_kpis(
    ...
    kpis = {
        ...
        "mean_yaw_rmse": float(np.nanmean(res["yaw_rmse"][res["yaw_ok"]]))
        if res["yaw_ok"].any()
        else np.nan,
        "mean_yaw_bias": float(np.nanmean(res["yaw_bias"][res["yaw_ok"]]))
        if res["yaw_ok"].any()
        else np.nan,
        "mean_mz_rmse": float(np.nanmean(res["mz_rmse"][res["mz_ok"]]))
        if res["mz_ok"].any()
        else np.nan,
        "mean_mz_bias": float(np.nanmean(res["mz_bias"][res["mz_ok"]]))
        if res["mz_ok"].any()
        else np.nan,
        "mean_ratio": float(np.nanmean(res["ratio"][res["ratio_ok"]]))
        if res["ratio_ok"].any()
        else np.nan,
        "mean_fb_share": float(np.nanmean(res["fb_share"][res["ratio_ok"]]))
        if res["ratio_ok"].any()
        else np.nan,
        "mean_corner_coverage_pct": (
            float(np.nanmean(res["cover"][res["any_ok"]]) * 100.0)
            if res["any_ok"].any()
            else np.nan
        ),
        "table": res["table"],
...
def yaw_moment_balance_fig(df: pl.DataFrame) -> tuple[go.Figure, dict]:
    ...
    valid = np.all(np.stack([np.isfinite(arr[c]) for c in cols], axis=1), axis=1) & np.isfinite(
        dist_m
    )
            ...
            fig.add_trace(
                go.Scattergl(
                    ...
                    line=dict(
                        color=color,
                        width=1.2,
                        dash=dash if lap_i == 0 else dash_cycle[lap_i % len(dash_cycle)],
                    ),
    ...
    if scatter_mask.any():
        fig.add_trace(
            go.Scattergl(
                ...
                mode="markers",
                marker=dict(
                    color=np.abs(arr[ay_col][scatter_mask]),
                    colorscale="Turbo",
                    size=4,
                    opacity=0.50,
                    colorbar=dict(title="|ay| [m/s²]"),
                ),
                name="Corner samples",
            ),
            row=2,
        ...
        lo = float(
            np.nanmin(
                [
                    np.nanmin(arr["TV_desiredMz"][scatter_mask]),
                    np.nanmin(arr["TV_actualMz"][scatter_mask]),
                ]
            )
        )
        hi = float(
            np.nanmax(
                [
                    np.nanmax(arr["TV_desiredMz"][scatter_mask]),
                    np.nanmax(arr["TV_actualMz"][scatter_mask]),
                ]
            )
        )
        fig.add_trace(
    ...
    fig.update_layout(
        title=dict(
            text=f"Yaw moment closed-loop diagnostic · Iz={IZ_KGM2:.3f} kg·m²",
    ...
    fig.update_xaxes(
        title_text="Distance [m]", color="#E5E5E5", gridcolor="rgba(128,128,128,0.2)", row=1, col=1
    )
    fig.update_yaxes(
        title_text="Mz [Nm]", color="#E5E5E5", gridcolor="rgba(128,128,128,0.2)", row=1, col=1
    )
    fig.update_xaxes(
        title_text="Desired Mz [Nm]",
        color="#E5E5E5",
    ...
    if scatter_mask.any():
        ...
        if has_limit:
            ...
            saturated_pct = float(
                np.nanmean(
                    (limit > 1.0) & (np.abs(arr["TV_actualMz"][scatter_mask]) >= 0.98 * limit)
                )
                * 100.0
            )
        else:
    ...
    kpis = {
        "tracking_rmse_nm": float(np.sqrt(np.nanmean(tracking_err**2)))
        if tracking_err.size
        else np.nan,
        "inertial_rmse_nm": float(np.sqrt(np.nanmean(inertial_err**2)))
        if inertial_err.size
        else np.nan,
        "saturated_pct": saturated_pct,
        "ff_over_fb_ratio": float(np.nanmean(ff_abs) / (np.nanmean(fb_abs) + EPS_RATIO))
        if ff_abs.size
        else np.nan,
        "fb_share": float(
            np.nanmean(fb_abs) / (np.nanmean(ff_abs) + np.nanmean(fb_abs) + EPS_RATIO)
        )
        if ff_abs.size
        else np.nan,
...
def tv_function_kpis(df: pl.DataFrame) -> tuple[list[go.Figure], dict]:
    ...
    if cm.any():
        ...
        bias_lr = (
            ...
            if np.isfinite(yaw_bias_left) and np.isfinite(yaw_bias_right)
            else np.nan
        )
    else:
    ...
    if cm.any():
        fig_err_ay.add_trace(
            go.Scattergl(
                x=np.abs(arr[ay_col][cm]),
                y=yaw_err[cm],
                mode="markers",
        ...
        fig_err_ay.add_hline(
            y=0.0, line=dict(color="rgba(200,200,200,0.6)", dash="dash", width=1.2)
        )
...
def yaw_rate_triple_fig(df: pl.DataFrame) -> tuple[go.Figure, dict]:
    ...
    """
    df = ensure_complete_laps_df(df)
    needed = ["TimeStamp", "laps", "laptime", "TV_desiredYawRate", "TV_errorYawRate", "Steering"]
    vx_col = _vx_signal(df.columns)
...
def tv_control_attribution_figs_kpis(df: pl.DataFrame) -> tuple[list[go.Figure], dict]:
    ...
    dist_m = lap_dist_from_gps(df)
    steering = (
        df["Steering"].to_numpy().astype(float)
    )  # steering-pot value [rad]; no STEERING_RATIO
    ay_col = _ay_signal(df.columns)
    vx_col = _vx_signal(df.columns)
    ...
    eval_mask = (
        corner_mask & np.isfinite(yaw_gain) & np.isfinite(actual_mz) & (np.abs(vx) >= MIN_SPEED)
    )
    ...
    entry_mask = eval_mask & (np.abs(steer_rate) > 0.08) & (np.abs(yaw_implied) > 0.08)
        ...
        lap_rows.append(
            {
                ...
                "Entry rotation [rad/s²]": round(
                    float(np.nanmedian(rotation_response[entry_lap])), 3
                )
                if entry_lap.any()
                else np.nan,
    ...
    fig_balance.add_trace(
        go.Scattergl(
            ...
            name="Yaw gain",
            marker=dict(
                color=np.abs(ay[order]),
                colorscale="Turbo",
                size=3,
                opacity=0.55,
                colorbar=dict(title="|ay|"),
            ),
        ),
        row=1,
        col=1,
    ...
    fig_balance.add_hrect(
        y0=0.90, y1=1.10, fillcolor="rgba(115,217,115,0.12)", line_width=0, row=1, col=1
    )
    fig_balance.add_hline(
        y=1.0, line=dict(color="rgba(235,235,235,0.7)", dash="dash", width=1.2), row=1, col=1
    )
    fig_balance.add_trace(
        go.Scattergl(
            x=dist_m[order],
            y=balance_error_pct[order],
            mode="markers",
            name="Balance error",
    ...
    fig_balance.add_hline(
        y=0.0, line=dict(color="rgba(235,235,235,0.7)", dash="dash", width=1.2), row=2, col=1
    )
    fig_balance.add_trace(
        go.Scattergl(
            x=dist_m[order],
    ...
    fig_balance.update_xaxes(
        title_text="Distance [m]", row=3, col=1, gridcolor="rgba(128,128,128,0.2)"
    )
    fig_balance.update_yaxes(
        title_text="Yaw gain [-]", row=1, col=1, range=[0.0, 2.0], gridcolor="rgba(128,128,128,0.2)"
    )
    fig_balance.update_yaxes(
        title_text="Balance error [%]", row=2, col=1, gridcolor="rgba(128,128,128,0.2)"
    )
    ...
    fig_rel.add_trace(
        go.Scattergl(
            ...
            name="ay -> yaw gain",
            marker=dict(
                color=actual_mz[rel_mask],
                colorscale="RdBu",
                size=4,
                opacity=0.45,
                colorbar=dict(title="Mz"),
            ),
        ),
        row=1,
        col=2,
    ...
    fig_rel.add_hline(
        y=0.0, line=dict(color="rgba(235,235,235,0.5)", dash="dash", width=1.0), row=1, col=1
    )
    fig_rel.add_hline(
        y=1.0, line=dict(color="rgba(235,235,235,0.5)", dash="dash", width=1.0), row=1, col=2
    )
    ...
    fig_rel.update_xaxes(
        title_text="TV actual Mz [Nm]", row=1, col=1, gridcolor="rgba(128,128,128,0.2)"
    )
    fig_rel.update_yaxes(
        title_text="Balance error [%]", row=1, col=1, gridcolor="rgba(128,128,128,0.2)"
    )
    fig_rel.update_xaxes(title_text="|ay| [m/s²]", row=1, col=2, gridcolor="rgba(128,128,128,0.2)")
    fig_rel.update_yaxes(
        title_text="Yaw gain [-]", row=1, col=2, range=[0.0, 2.0], gridcolor="rgba(128,128,128,0.2)"
    )

    kpis = {
        "corner_samples": int(eval_mask.sum()),
        "yaw_gain_median": float(np.nanmedian(yaw_gain[eval_mask])) if eval_mask.any() else np.nan,
        "understeer_pct": float(np.nanmean(understeer[eval_mask]) * 100.0)
        if eval_mask.any()
        else np.nan,
        "oversteer_pct": float(np.nanmean(oversteer[eval_mask]) * 100.0)
        if eval_mask.any()
        else np.nan,
        "balanced_pct": float(np.nanmean(balanced[eval_mask]) * 100.0)
        if eval_mask.any()
        else np.nan,
        "entry_rotation_radss": float(np.nanmedian(rotation_response[entry_mask]))
        if entry_mask.any()
        else np.nan,
...
def tv_corner_under_oversteer_figs_kpis(
    ...
    *,
            ...
            turn_rows.append(
                {
                    ...
                    "TV Mz [Nm]": round(float(np.nanmedian(actual_mz[tm])), 1)
                    if np.isfinite(actual_mz[tm]).any()
                    else np.nan,
        ...
        turn_summary_rows.append(
            {
                ...
                "Distance [m]": round(
                    float(np.nanmedian([float(row["Distance [m]"]) for row in rows])), 1
                ),
                "Laps": int(len(rows)),
                "Worst lap": int(
                    rows[int(np.nanargmax(np.abs([float(row["Balance [%]"]) for row in rows])))][
                        "Lap"
                    ]
                ),
            }
    ...
    if turn_summary_rows:
        ...
        colors = [
            "#4DB3F2"
            if value < -YAW_BALANCE_NEUTRAL_BAND_PCT
            else "#F28C40"
            if value > YAW_BALANCE_NEUTRAL_BAND_PCT
            else "#D8D8D8"
            for value in y
    ...
    kpis = {
        ...
        "understeer_corners_pct": float(np.nanmean(under_corners) * 100.0)
        if score_values.size
        else np.nan,
        "oversteer_corners_pct": float(np.nanmean(over_corners) * 100.0)
        if score_values.size
        else np.nan,
        "neutral_corners_pct": float(np.nanmean(neutral_corners) * 100.0)
        if score_values.size
        else np.nan,
        "sample_understeer_pct": float(np.nanmean(understeer[eval_mask]) * 100.0)
        if eval_mask.any()
        else np.nan,
        "sample_oversteer_pct": float(np.nanmean(oversteer[eval_mask]) * 100.0)
        if eval_mask.any()
        else np.nan,
